...
function parseArgs(args: string[]): Opts {
  ...
  return { out, channel, since, types };
}
...
export async function runExport(args: string[]) {
  ...
  await write({
    ...
    version: VERSION,
    filters: { channel: opts.channel ?? null, since: sinceIso, types: [...opts.types] },
  });
  ...
  if (opts.types.has("screen")) {
    let sql = `SELECT timestamp, pid, window_index, app, window_title, texts, channel_names, window_id, diff_text FROM screen_states WHERE timestamp > ?`;
  ...
  if (opts.types.has("audio")) {
    const sql = `SELECT timestamp, audio_path, transcript FROM audio_transcripts WHERE timestamp > ? ORDER BY timestamp ASC`;
    for (const r of db.prepare(sql).iterate(sinceIso) as any) {
  ...
  if (opts.types.has("ingested")) {
    let sql = `SELECT timestamp, source, channel_name, text, meta FROM ingested WHERE timestamp > ?`;
    const params: any[] = [sinceIso];
    if (opts.channel) {
